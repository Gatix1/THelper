...
def get_darwin_laptops(db, cursor, url, page_num_class):
    ...
    for page in range(1, pages_num+1):
        ...
        product_raw_titles = soup.select('.info-wrap')
        product_prices = soup.find_all('span', attrs={"class": ["price-new"]})
        image_links = soup.select('.card-image')
        product_descrs = []
        product_titles = []
        ...
        for i in range(len(product_raw_titles)):
            ...
            if '|' in product_raw_descr.text:
                ...
                try:
                    ...
                    if 'TB' in temp_descr[5]:
                        temp_descr[5] = str(int(temp_descr[5].replace('TB', '').strip()) * 1024)
                    product_descr.append(temp_descr[2].strip())
                    product_descr.append("")
                    product_descr.append(temp_descr[-4].strip())
                    if int(temp_descr[-2].strip()) < 3:
                        product_descr.append(str(int(temp_descr[-2].strip())*1024))
                    else:
                        product_descr.append(str(temp_descr[-2].strip()))
        ...
        for i in range(len(product_titles)):
            image_url = image_links[i].get('src')
            image_response = requests.get(image_url)
            with open(f"static/imgs/{product_titles[i].text.replace('Laptop ', '').replace('/', '-')}.jpg", "wb") as f:
                f.write(image_response.content)
            product_title = product_titles[i].text.replace('/', '-').replace('Laptop ', '')
            specs = {
                'TITLE': product_title,
...
def get_enter_phones(db, cursor, url, page_num_class):
    ...
    for page in range(1, pages_num):
        ...
        for i in range(len(product_descr)):
            mem = int(re.findall(r"\d\d*\s", product_descr[i].text)[1].replace("GB", "").strip())
            if mem < 2:
                mem *= 1024
            image_url = image_link[i].find('img').get('data-src')
            image_response = requests.get(image_url)
            ...
            specs = {
                'TITLE': product_titles[i].text,
                'PRICE': product_prices[i].text.replace(" ", "").replace("lei", ""),
                'RAM': str(re.findall(r"\d\d*\s", product_descr[i].text)[0].strip()),
                'MEMORY': mem,
                'LINK': product_titles[i].find_parent('a').get('href'),
                'SOURCE': 'ENTER.online'
            }
...
def get_phones():
    print("Started parsing smartphones...")
    start = timer()
    ...
    print("Smartphones successfully parsed!")
    print(str(int(end-start)) + " seconds elapsed for parsing smartphones!")


def get_laptops():
    print("Started parsing laptops...")
    start = timer()
    ...
    print("Laptops successfully parsed!")
    print(str(int(end-start)) + " seconds elapsed for parsing laptops!")
